...
mod tests {
    ...
    #[test]
    fn parses_elm_import_ids() {
        let parsed = import_id::parse("/src/Main.elm?with=./Other.elm&with=./Third.elm").unwrap();
        assert_eq!(parsed.path, "/src/Main.elm");
        assert_eq!(parsed.with, vec!["./Other.elm", "./Third.elm"]);
        assert!(parsed.valid);
        assert!(!import_id::parse("/src/Main.elm?raw").unwrap().valid);
    }
    ...
    fn converts_elm_iife_to_esm() {
        ...
        let out = esm::to_es_module(js).unwrap();
    ...
    #[test]
    fn injects_asset_imports() {
        let out = assets::inject("const x = '[VITE_PLUGIN_ELM_ASSET:/assets/logo.png]';").unwrap();
